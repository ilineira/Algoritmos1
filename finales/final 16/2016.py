...
def procesar_mudanza(salida,llegada,faltantes):
    ...
    while es_valida(linea) and es_valida(linea2):
        ...
        while es_valida(linea):
            ...
            linea = archsalida.readline()
            print("Peso total: {0}/{1}​\n​Vol. total: {2}/{3}".format(s_peso_total, l_peso_total, s_vol_total, l_vol_total))

#if ​__name__ == '__main__'​:
with open("faltantes.txt", "w") as faltantes, open("salida.csv", "r") as archsalida, open("llegada.csv", "r") as archllegada:
    procesar_mudanza(archsalida, archllegada, faltantes)